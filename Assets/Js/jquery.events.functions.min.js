 ...
 *
 * Events specific setup and functions. Also minified for your viewing pleasure.
...
$(document).ready(function(){$('.category h3, .arena h3').live('click',function(){if(!$(this).parent().find('div ul').hasClass('open')){$('.category ul, .arena ul').removeClass('open').slideUp(150)}$(this).parent().find('div ul').addClass('open').slideToggle(150)});$(document).live('click',function(e){if($(e.target).closest('li',this).parent().length==0){$('.category ul, .arena ul').removeClass('open').slideUp(150)}});$('.category li, .arena li').live('click',function(){$(this).parent().removeClass('open').slideToggle(150);$(this).parents().closest('li').find('h3').html($(this).text()+'<i></i>');if(!$(this).hasClass('active')){$('.timeline li a').css({'margin':''});klapp_filter_events($(this))}});$('.timeline li a').hover(function(){if($(this).parent().hasClass('odd')){$(this).animate({'margin-right':'-10px'},{queue:false,duration:200})}else{$(this).animate({'margin-left':'-10px'},{queue:false,duration:200})}},function(){if($(this).parent().hasClass('odd')){$(this).animate({'margin-right':'0px','margin-left':'11px'},{queue:false,duration:200})}else{$(this).animate({'margin-left':'0px'},{queue:false,duration:200})}});$('.module.event ol li').live('click',function(){var event_time=$(this).attr('data-event-time');if(!$(this).hasClass('active')&&!$('.module.event > div').is(':animated')){$('.module.event ol').children().removeClass('active');$(this).addClass('active');$('.module.event > div').each(function(){if(event_time==$(this).attr('data-event-time')){$('.module.event > div.active').removeClass('active').hide();$(this).fadeIn(400,function(){$(this).addClass('active')});return false}})}});$('.second-row li, .third-row li').each(function(){var height=$('p',this).height()+10;$('p',this).css({'bottom':'-'+height+'px'})});$('.second-row li, .third-row li').hover(function(){var height=$('p',this).height()+10;$('h3, p',this).stop(true,true).animate({'bottom':'+='+height+'px'},{duration:160})},function(){var height=$('p',this).height()+10;$('h3, p',this).stop(true,true).animate({'bottom':'-='+height+'px'},{duration:360})});if($('body').hasClass('ie7')){function ie_fix(){return true}window.onerror=ie_fix}});

//window load events
$(window).load(function(){$('.timecapsule').each(function(){$(this).attr('data-element-height',$(this).height());$('li',this).each(function(){$(this).attr('data-element-height',$(this).height())})});klapp_expand_events_onload()});
